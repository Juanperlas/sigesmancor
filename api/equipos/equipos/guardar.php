...
// Validar que tipo_equipo no esté vacío y tenga una longitud razonable
if (empty($datos['tipo_equipo']) || strlen($datos['tipo_equipo']) > 100) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'El tipo de equipo debe tener entre 1 y 100 caracteres']);
    exit;
}
...
try {
    // Conexión a la base de datos - MOVER AQUÍ ANTES DE LAS VALIDACIONES QUE LA NECESITEN
    $conexion = new Conexion();

    // Validar que la categoría existe - AHORA SÍ PODEMOS USAR $conexion
    $categoriaExiste = $conexion->selectOne("SELECT id FROM categorias_equipos WHERE id = ?", [$datos['categoria_id']]);
    if (!$categoriaExiste) {
        http_response_code(400);

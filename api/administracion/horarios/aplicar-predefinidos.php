 ...
 */
function registrarEnHistorialTrabajo($conexion, $equipoId, $componenteId, $horas, $fuente = 'automatico') {
    $fechaHoy = date('Y-m-d');
...
try {
    ...
    foreach ($input['registros'] as $registro) {
        if (!isset($registro['id'], $registro['tipo'], $registro['horas'], $registro['seleccionado']) || 
        ...
        if ($actual) {
            ...
            if ($resultado) {
                ...
                $resultadoHistorial = registrarEnHistorialTrabajo($conexion, $equipoId, $componenteId, $horas, 'automatico');
    ...
    echo json_encode([
        'success' => true,
        'message' => "Se aplicaron las horas predefinidas a $registrosActualizados registros correctamente y se registraron $registrosHistorial entradas en el historial",

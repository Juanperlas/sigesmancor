...
try {
    ...
    if (!$persona) {
        ob_end_clean();
        header("Location: ../../../dashboard.php?error=personal_no_encontrado");
        exit;
    }

    // Verificar fotografía
    $fotografiaPath = !empty($persona['imagen']) && file_exists('../../../' . $persona['imagen'])
        ? '../../../' . $persona['imagen']
        : '../../../assets/img/administracion/personal/default.png';

    $fotografiaInfo = getImageInfo($fotografiaPath);
    $fotografiaPath = $fotografiaInfo['path'] ?: '../../../assets/img/administracion/personal/default.png';
    ...
    $pdf->SetAuthor($autor);
    $pdf->SetTitle('Informe de Personal - ' . $persona['nombre']);
    $pdf->SetSubject('Informe generado desde SIGESMANCOR');
    $pdf->SetKeywords('personal, informe, SIGESMANCOR');
    ...
    $pdf->SetXY(12, 10);
    $pdf->Cell(0, 8, 'INFORME DE PERSONAL', 0, 1, 'L');
    ...
    // Fotografía del personal circular
    ...
    $pdf->SetTextColor(50, 50, 50);
    $pdf->SetXY(50, 48);
    $pdf->Cell(100, 6, strtoupper($persona['nombre']), 0, 1, 'L');

    $pdf->SetFont($fontname, '', 11);
    $pdf->SetTextColor(21, 113, 176);
    $pdf->SetXY(50, 56);
    $pdf->Cell(100, 5, $persona['area'] ?: 'Sin área asignada', 0, 1, 'L');

    // Estado con badge corporativo
    $estado = $persona['esta_activo'] ? 'ACTIVO' : 'INACTIVO';
    $estadoColor = $persona['esta_activo'] ? array(32, 201, 151) : array(230, 57, 70); // success/danger
    ...
    $y = addCompactInfoRow($pdf, 'Nombre Completo', $persona['nombre'], $fontname);
    $y = addCompactInfoRow($pdf, 'DNI', empty($persona['dni']) ? 'No especificado' : $persona['dni'], $fontname);
    $y = addCompactInfoRow($pdf, 'Teléfono', empty($persona['telefono']) ? 'No especificado' : $persona['telefono'], $fontname);
    $y = addCompactInfoRow($pdf, 'Dirección', empty($persona['direccion']) ? 'No especificada' : $persona['direccion'], $fontname, true);

    $pdf->Ln(5);

    // === SECCIÓN INFORMACIÓN DE CONTACTO ===
    $y = addCompactSectionHeader($pdf, 'INFORMACIÓN LABORAL', '💼', $fontname, [156, 39, 176]); // Púrpura vibrante
    ...
    // Filas de información
    $y = addCompactInfoRow($pdf, 'Área', empty($persona['area']) ? 'No especificada' : $persona['area'], $fontname);
    $y = addCompactInfoRow($pdf, 'Fecha de Ingreso', empty($persona['fecha_ingreso']) ? 'No especificada' : date('d/m/Y', strtotime($persona['fecha_ingreso'])), $fontname);
    $y = addCompactInfoRow($pdf, 'Fecha de Baja', empty($persona['fecha_baja']) ? 'No especificada' : date('d/m/Y', strtotime($persona['fecha_baja'])), $fontname, true);

    $pdf->Ln(5);

    // === SECCIÓN ESTADO DE LA CUENTA ===
    $y = addCompactSectionHeader($pdf, 'ESTADO DEL REGISTRO', '⚡', $fontname, [76, 175, 80]); // Verde vibrante
    ...
    // Filas de información
    $y = addCompactInfoRow($pdf, 'Estado Actual', $persona['esta_activo'] ? 'Activo' : 'Inactivo', $fontname);
    $y = addCompactInfoRow($pdf, 'Creado por', isset($persona['creado_por_nombre']) ? $persona['creado_por_nombre'] : 'No disponible', $fontname);
    $y = addCompactInfoRow($pdf, 'Fecha de Registro', isset($persona['creado_en']) ? date('d/m/Y H:i:s', strtotime($persona['creado_en'])) : 'No disponible', $fontname, true);
    ...
    $nombreArchivo = 'informe_personal_' . strtolower(str_replace(' ', '_', $persona['nombre'])) . '_' . date('Y-m-d') . '.pdf';

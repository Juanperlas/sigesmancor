...
try {
    ...
    $mantenimiento = $conexion->selectOne(
        "SELECT mc.*, 
                ...
                CASE 
                    ...
                    WHEN mc.componente_id IS NOT NULL THEN c.orometro_actual
                END as orometro_actual,
                CASE 
                    WHEN mc.equipo_id IS NOT NULL THEN e.anterior_orometro
                    WHEN mc.componente_id IS NOT NULL THEN c.anterior_orometro
    ...
    if ($mantenimiento['equipo_id']) {
        $historial = $conexion->select(
            "SELECT hm.*, 'correctivo' as origen
             FROM historial_mantenimiento hm
             WHERE hm.equipo_id = ? AND hm.tipo_mantenimiento = 'correctivo'
             UNION ALL
             SELECT mc.id, mc.descripcion_problema as descripcion, mc.fecha_realizado, 
                    mc.orometro_actual as orometro_realizado, mc.observaciones, mc.imagen, 
                    mc.creado_en, 'correctivo' as tipo_mantenimiento, 
                    mc.id as mantenimiento_id, mc.equipo_id, NULL as componente_id, 'actual' as origen
             FROM mantenimiento_correctivo mc
        ...
        $historial = $conexion->select(
            "SELECT hm.*, 'correctivo' as origen
             FROM historial_mantenimiento hm
             WHERE hm.componente_id = ? AND hm.tipo_mantenimiento = 'correctivo'
             UNION ALL
             SELECT mc.id, mc.descripcion_problema as descripcion, mc.fecha_realizado, 
                    mc.orometro_actual as orometro_realizado, mc.observaciones, mc.imagen, 
                    mc.creado_en, 'correctivo' as tipo_mantenimiento, 
                    mc.id as mantenimiento_id, NULL as equipo_id, mc.componente_id, 'actual' as origen
             FROM mantenimiento_correctivo mc
    ...
    $imagenPath = $imagenInfo['path'] ?: '../../../assets/img/mantenimiento/correctivo/default.png';
    ...
    $pdf->SetAuthor($autor);
    $pdf->SetTitle('Informe de Mantenimiento Correctivo - ' . $mantenimiento['codigo_item']);
    $pdf->SetSubject('Informe generado desde SIGESMANCOR');
    $pdf->SetKeywords('mantenimiento, correctivo, informe, SIGESMANCOR');
    ...
    $pdf->SetXY(12, 10);
    $pdf->Cell(0, 8, 'INFORME DE MANTENIMIENTO CORRECTIVO', 0, 1, 'L');
    ...
    $y = addCompactInfoRow($pdf, 'Descripción Problema', $mantenimiento['descripcion_problema'] ?: 'No especificada', $fontname);
    ...
    $y = addCompactInfoRow($pdf, 'Orómetro Actual', number_format($mantenimiento['orometro_actual'], 2) . ' ' . $unidad, $fontname, true);
    ...
    $nombreArchivo = 'informe_mantenimiento_correctivo_' . strtolower(str_replace(' ', '_', $mantenimiento['codigo_item'])) . '_' . date('Y-m-d') . '.pdf';
